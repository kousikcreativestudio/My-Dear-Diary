...
onAuthStateChanged(auth, async (user) => {
  const accountChanged = lastKnownUid !== null && (!user || user.uid !== lastKnownUid);

  /* BUGFIX: the Vault's unlocked state (currentVaultMode, the in-memory
     encryption password, and already-rendered decrypted secrets in
     #vaultEntries) previously survived a logout or switch to a different
     account. If the vault was open/unlocked when the user switched accounts,
     the new account would briefly display the old account's vault content,
     and the leftover state caused confusing "Unlock vault again to enable
     encryption" prompts. Force the vault fully closed and cleared any time
     the signed-in account changes. */
  if (accountChanged) {
    if (typeof window.closeVaultModal === "function") window.closeVaultModal();
    if ($("vaultArea")) $("vaultArea").classList.remove("active");
  ...
  lastKnownUid = user ? user.uid : null;
...
function sanitizeEditorHTML(html) {
  ...
  const allowedTags = new Set([
    ...
    "UL", "OL", "LI", "BLOCKQUOTE", "H1", "H2", "H3", "H4", "HR", "A", "IMG"
  ]);
  ...
  const allowedStyleProperties = new Set([
    ...
    "background-color", "text-align", "text-decoration", "text-decoration-line",
  ...
  [...box.querySelectorAll("*")].forEach((element) => {
    ...
    [...element.attributes].forEach((attribute) => {
      ...
      if (name.startsWith("on") || !["style", "href", "src", "alt", "target", "rel", "class"].includes(name)) {
    ...
    if (rawStyle && !/url\s*\(|expression\s*\(|javascript\s*:/i.test(rawStyle)) {
      ...
      allowedStyleProperties.forEach((property) => {
        const value = styleProbe.style.getPropertyValue(property).trim();
        if (value && value.length <= 120) element.style.setProperty(property, value);
      });
    }
    ...
    if (element.tagName === "IMG") {
      ...
      const safeImage = /^(https?:|data:image\/(?:png|jpeg|jpg|gif|webp);base64,)/i.test(src);
      if (!safeImage) {
        element.remove();
        return;
      }
      element.setAttribute("alt", (element.getAttribute("alt") || "Diary image").slice(0, 200));
    } else {
    ...
    if (element.tagName === "HR") {
      element.className = "entry-page-break";
    } else {
...
function validateMediaFile(file) {
  ...
  if (file.size > MAX_MEDIA_BYTES) {
    throw new Error("Media must be smaller than 25 MB.");
  }
}
...
async function uploadUserMedia(file, folder) {
  if (!file) return { mediaUrl: "", mediaPath: "" };
  ...
  const path = `users/${currentUser.uid}/${folder}/${Date.now()}-${uniqueId}-${safeStorageFileName(file.name)}`;
  const target = storageRef(storage, path);
  await uploadBytes(target, file, { contentType: file.type || "application/octet-stream" });
  return { mediaUrl: await getDownloadURL(target), mediaPath: path };
}

async function deleteStoredMedia(path) {
  if (!path) return;
  try {
...
window.saveEntry = async function () {
  ...
  let uploadedMedia = { mediaUrl: "", mediaPath: "" };
  try {
    uploadedMedia = await uploadUserMedia(mediaFile, "diary");
    ...
    if (uploadedMedia.mediaPath) await deleteStoredMedia(uploadedMedia.mediaPath);
    alert(error.message);
  }
};
...
window.deleteEntry = async function (id) {
  ...
  try {
    ...
    if (entry?.mediaPath) await deleteStoredMedia(entry.mediaPath);
    await loadEntries();
  } catch (error) {
...
window.saveEditedEntry = async function () {
  ...
  const editedHTML = sanitizeEditorHTML($("editEntryText").innerHTML.trim());

  /* BUGFIX: previously only `text` was updated here, while the main entries
     list (renderEntryPages) displays from `pages` whenever it exists -- so
     edits to a multi-page entry silently failed to show up in "My Magical
     Pages". Split the edited HTML back into pages on the same <hr> boundary
     used when pages are first saved, and write both fields together so they
     stay in sync. */
  const pages = editedHTML
    .split(/<hr[^>]*>/i)
    .map((p) => p.trim())
    .filter((p) => stripHTML(p));

  const finalPages = pages.length ? pages : [editedHTML];

  await updateDoc(doc(db, "diaryEntries", editingEntryId), {
    title: $("editEntryTitle").value.trim() || "Untitled Diary",
    text: finalPages.join("<hr class='entry-page-break'>"),
    pages: finalPages,
...
window.saveProfile = function () {
  ...
  window.loadProfile(currentUser);
  closeProfileModal();
};
...
function renderMedia(entry){
  ...
  const mediaUrl = safeMediaURL(entry.mediaUrl, entry.mediaType);
  if(!mediaUrl) return "";

  if((entry.mediaType || "").startsWith("video")){
    return `
      <div class="memory-media-card">
        <video controls src="${escapeHTML(mediaUrl)}" class="memory-media"></video>
      </div>
    `;
  }
  ...
  return `
    <div class="memory-media-card">
      <img src="${escapeHTML(mediaUrl)}" alt="Memory photo" class="memory-media"
        data-media-url="${escapeHTML(mediaUrl)}" onclick="openMediaViewer(this.dataset.mediaUrl, 'image')">
    </div>
  `;
}
...
window.previewMemoryMedia = function(){
  ...
  if(file.size > MAX_MEDIA_BYTES){
    alert("Photo/video must be smaller than 25 MB.");
    input.value = "";
    box.innerHTML = "";
    if(removeBtn) removeBtn.style.display = "none";
    return;
...
window.openMediaViewer = function(url, type){
  const safeUrl = safeMediaURL(url, type === "video" ? "video/*" : "image/*");
  if(!safeUrl) return;
  ...
  viewer.innerHTML = `
    <div class="media-viewer-content">
      ...
      ${type === "video"
        ? `<video controls autoplay src="${escapeHTML(safeUrl)}"></video>`
        : `<img src="${escapeHTML(safeUrl)}" alt="Memory preview">`
      }
    </div>
  `;
...
function safeMediaURL(value, mediaType = "") {
  ...
  if ((mediaType || "").startsWith("video") && /^data:video\/[a-z0-9.+-]+;base64,/i.test(url)) return url;
...
window.exportDiaryPDF = function(){
  ...
  entries.forEach((e, i)=>{
    ...
    html += `<div>${sanitizeEditorHTML(e.text || "")}</div>`;
  });
...
function renderSlide(){
  ...
  box.innerHTML = `
    <div class="cinematic-slide">
      ...
      <div>${sanitizeEditorHTML(e.text || "")}</div>${renderMedia(e)}
...
window.previewVaultMedia = function(){
  ...
  if(file.size > MAX_MEDIA_BYTES){
    alert("Secret photo/video must be smaller than 25 MB.");
    input.value = "";
    box.innerHTML = "";
    if(removeBtn) removeBtn.style.display = "none";
    return;
...
function renderVaultMedia(entry){
  ...
  return `
    <div class="memory-media-card vault-secret-media">
      <img src="${escapeHTML(mediaUrl)}" alt="Secret memory" class="memory-media"
        data-media-url="${escapeHTML(mediaUrl)}" onclick="openMediaViewer(this.dataset.mediaUrl, 'image')">
    </div>
  `;
}
...
window.saveVaultEntry = async function(){
  ...
  const uploadedMedia = await uploadUserMedia(mediaFile, "vault");
  const mediaUrl = uploadedMedia.mediaUrl;
...
window.loadVaultEntries = async function(filterType = "all"){
  ...
  $("vaultEntries").innerHTML = list.length ? list.map(e=>`
    <div class="vault-entry vault-entry-card">
      ...
      ${e.text ? `<div>${sanitizeEditorHTML(e.text)}</div>` : ""}
      ${renderVaultMedia(e)}
...
function buildPrintableDiaryHTML(entries, includePhotos, title){
  ...
  const pages = entries.map((entry, index)=>{
    ...
    const media = includePhotos && entry.mediaUrl && !(entry.mediaType || "").startsWith("video")
      ? `<img class="pdf-photo" src="${escapeHTML(safeMediaURL(entry.mediaUrl, entry.mediaType))}" alt="Memory photo">`
      : "";
    ...
    return `
      <section class="pdf-entry">
        ...
        ${media}
        <div class="pdf-text">${sanitizeEditorHTML(entry.text || "")}</div>
...
/* BUGFIX: app.js is loaded as a module, so this function was never reachable
   from the inline onclick/oninput handlers in the HTML (timeline search box
   and the "Back to Timeline" button) without being attached to window. */
window.renderPremiumTimeline = renderPremiumTimeline;
...
function renderEntryPages(entry){
  ...
  if(pages.length <= 1){
    return `<div>${sanitizeEditorHTML(pages[0] || "")}</div>`;
  }
    ...
    <div class="entry-pages-view">
      ${pages.map((page, i)=>`
        <div class="entry-page-card">
          <h4>📄 Page ${i + 1}</h4>
          <div>${sanitizeEditorHTML(page || "")}</div>
        </div>
      `).join("")}
    </div>
  `;
...
function renderTimeCapsuleCard(c){
  ...
  const content = c.unlocked
    ? `<div class="capsule-content">${sanitizeEditorHTML(c.text || "")}</div>`
...
async function getCollectionBackup(collectionName){
  ...
  try{
    ...
    throw new Error(`Could not read ${collectionName}: ${error.message}`);
  }
  return out;
}
...
window.unlockVaultWithFingerprint = async function(){
  ...
  try{
    ...
    /* BUGFIX: previously currentVaultMode was never set to "real" here, so
       Save Secret / Change Password incorrectly alerted "Unlock real vault
       first" even though the vault was visibly open. Fingerprint can prove
       identity but cannot recover the encryption key on its own, so encrypted
       secrets still need the vault password once per session to decrypt --
       that part already degrades gracefully (per-entry "Cannot decrypt"
       message) and is left as-is. */
    if(typeof setVaultMode === "function") setVaultMode("real");
    if($("vaultModeLabel")) $("vaultModeLabel").style.display = "block";

    await loadVaultEntries("all");

    setVaultFingerprintStatus(
      activeVaultEncryptionPassword
        ? "Vault unlocked with fingerprint."
        : "Vault unlocked with fingerprint. Enter your vault password once this session to decrypt encrypted secrets.",
      "success"
    );
  }catch(error){
...
window.filterGallery=function(type){
 ...
 const items=(allEntries||[]).filter(e=>safeMediaURL(e.mediaUrl, e.mediaType));
 let list=items;
 ...
 const mediaUrl=safeMediaURL(e.mediaUrl,e.mediaType);
 return `<div class="gallery-item">
 ${(e.mediaType||'').startsWith('video')
 ? `<video controls src="${escapeHTML(mediaUrl)}"></video>`
 : `<img src="${escapeHTML(mediaUrl)}" data-media-url="${escapeHTML(mediaUrl)}" onclick="openMediaViewer(this.dataset.mediaUrl,'image')">`}
 <small>${escapeHTML(e.title||'Memory')}</small>
 </div>`;
 }).join('') || '<div>No media memories found.</div>';
}
...
function updateVaultPasswordUI(){
  ...
  const hasReal = vaultAnyPasswordExists();
...
window.deleteVaultEntry = async function(id, filterType = "all"){
  ...
  if(String(id).startsWith("local_vault_")){
    const localEntries = getLocalVaultEntries();
    const existing = localEntries.find(e => e.id === id);
    if(existing){
      const readable = await decryptVaultEntryForSafeDisplay(existing);
      mediaPath = readable.mediaPath || "";
    }
    saveLocalVaultEntries(localEntries.filter(e => e.id !== id));
  }else{
    try{
      const vaultRef = doc(db, "vaultEntries", id);
      const snapshot = await getDoc(vaultRef);
      if(snapshot.exists()){
        const readable = await decryptVaultEntryForSafeDisplay({id, ...snapshot.data()});
        mediaPath = readable.mediaPath || "";
      }
  ...
  if(mediaPath) await deleteStoredMedia(mediaPath);
  await loadVaultEntries(filterType);
};
...
window.openAchievementsModal = function(){
  window.checkAchievements(false);
...
if(typeof oldLoadEntriesForAchievements === "function"){
  window.loadEntries = async function(){
    ...
    setTimeout(()=>window.checkAchievements(true), 450);
  };
}
...
if(typeof oldOpenMemoryJourneyModalForAchievements === "function"){
  window.openMemoryJourneyModal = function(){
    ...
    setTimeout(()=>window.checkAchievements(true), 350);
  };
}
...
setTimeout(()=>window.checkAchievements(false), 1500);
...
if(typeof oldSaveVaultEntryForEncryption === "function"){
  window.saveVaultEntry = async function(){
    ...
    if(!stripHTML(text) && !mediaFile) return alert("Write secret memory or add photo/video");
    ...
    try{
      ...
      encryptedPayload = await encryptVaultPayload(plainPayload, activeVaultEncryptionPassword);
    }catch(error){
      if(uploadedMedia.mediaPath) await deleteStoredMedia(uploadedMedia.mediaPath);
      return alert("Secret could not be prepared: " + error.message);
    }
    ...
    if(!savedToCloud){
      try{
        ...
        if(uploadedMedia.mediaPath) await deleteStoredMedia(uploadedMedia.mediaPath);
        return alert("Secret could not be saved: " + error.message);
      }
    }
...
async function rotateVaultEncryptionPassword(oldPassword, newPassword){
  const cloudSnapshot = await getDocs(
    query(collection(db, "vaultEntries"), where("uid", "==", currentUser.uid))
  );
  const encryptedCloudEntries = [];
  cloudSnapshot.forEach((snapshot) => {
    const data = snapshot.data();
    if(data.encrypted && data.vaultData){
      encryptedCloudEntries.push({id:snapshot.id, data});
    }
  });

  if(encryptedCloudEntries.length > 450){
    throw new Error("This vault has too many encrypted entries to rotate safely in one operation. Export a backup first.");
  }

  const preparedCloud = [];
  for(const entry of encryptedCloudEntries){
    const plain = await decryptVaultPayload(entry.data.vaultData, oldPassword);
    preparedCloud.push({
      id:entry.id,
      vaultData:await encryptVaultPayload(plain, newPassword)
    });
  }
  ...
  for(const entry of localEntries){
    if(entry.encrypted && entry.vaultData){
      const plain = await decryptVaultPayload(entry.vaultData, oldPassword);
      preparedLocal.push({
        ...entry,
        vaultData:await encryptVaultPayload(plain, newPassword)
      });
    }else{
      preparedLocal.push(entry);
    }
  }

  if(preparedCloud.length){
    const batch = writeBatch(db);
    preparedCloud.forEach((entry) => {
      batch.update(doc(db, "vaultEntries", entry.id), {
        vaultData:entry.vaultData,
        passwordRotatedAt:serverTimestamp()
      });
    });
    await batch.commit();
  }

  saveLocalVaultEntries(preparedLocal);
}

/* Change the verifier and re-encrypt every existing encrypted secret. */
window.changeVaultPassword = async function(){
  if(!currentUser) return alert("Login first");
  ...
  try{
    await rotateVaultEncryptionPassword(oldPass, newPass);
    await saveVaultPasswordHashes(newPass, newFake);
    activeVaultEncryptionPassword = newPass;
  }catch(error){
    return alert("Password was not changed: " + error.message);
  }
  ...
  updateVaultHashUI();
  alert("Vault password changed and existing secrets were re-encrypted.");
};
...
window.unlockAutoLock = async function(){
  if(!currentUser) return;

  const passwordInput = $("autoLockPassword");
  const password = passwordInput?.value || "";
  const providers = (currentUser.providerData || []).map(item => item.providerId);
  const unlockButton = $("autoLockUnlockButton");
  ...
  try{
    if(password && currentUser.email){
      ...
      throw new Error("Enter your account password to unlock the diary.");
    }
...
function collectBackupLocalStorage(){
  ...
  const exactKeys = new Set([
    "diaryTheme", "primaryColor", "accentColor", "bgColor", "fontChoice",
    "lampColor", "premiumThemeStore", "journeyAchievementUsed", "backupAchievementUsed"
  ]);
  const userPrefixes = [
    "profileName_", "profilePhoto_", "myDearDiaryVaultEntries_",
    "myDearDiaryAchievements_", "myDearDiaryRewards_", "myDearDiaryActiveRewardTheme_",
    "myDearDiaryActiveRewardBadge_", "myDearDiaryStreakRewards_", "vaultHashSalt_",
    "vaultHashReal_", "vaultHashFake_", "autoLockEnabled_"
  ];

  Object.keys(localStorage).forEach((key) => {
    if(key === capsuleLocalKey()) return;
    if(key.startsWith("myDearDiaryVaultFingerprint_")) return;
    if(key.startsWith("vaultPass_") || key.startsWith("vaultFakePass_")) return;
    if(key.startsWith("vaultWrongAttempts_") || key.startsWith("vaultLockoutUntil_")) return;
    if(key.startsWith("autoLockLastActivity_")) return;

    const isAllowedUserKey = userPrefixes.some(
      prefix => key === prefix + currentUser.uid
    );
    if(exactKeys.has(key) || isAllowedUserKey){
...
function restoreBackupLocalStorage(values, sourceUid){
  ...
  Object.entries(values).forEach(([originalKey, value]) => {
    if(value === null || value === undefined) return;
    let key = String(originalKey);

    if(sourceUid && key.endsWith(sourceUid)){
      key = key.slice(0, -sourceUid.length) + currentUser.uid;
    }

    const allowed = exactKeys.has(key) || allowedPrefixes.some(
      prefix => key === prefix + currentUser.uid
    );
    if(!allowed) return;

    const stringValue = String(value);
    if(["primaryColor", "accentColor", "bgColor", "lampColor"].includes(key)
      && !/^#[0-9a-f]{6}$/i.test(stringValue)) return;
    if(key === "diaryTheme"
      && !["dark", "light", "blue", "pink", "purple"].includes(stringValue)) return;
    if(key === "fontChoice"
      && !["Inter", "Poppins", "Georgia", "Trebuchet MS", "Courier New", "Comic Sans MS"].includes(stringValue)) return;

    localStorage.setItem(key, stringValue);
  });
}

/* Export every diary collection and the safe device settings inside one encrypted file. */
async function secureExportDiaryBackupCore(){
  ...
  const backupData = {
    ...
    vaultEntries:await getCollectionBackup("vaultEntries"),
    timeCapsules:await getCollectionBackup("timeCapsules"),
    localTimeCapsules:getLocalCapsules(),
  ...
  if(typeof window.checkAchievements === "function"){
    setTimeout(()=>window.checkAchievements(true), 100);
  }
...
window.exportDiaryBackup = async function(){
  try{
    return await secureExportDiaryBackupCore();
  }catch(error){
...
/* Restore both current encrypted backups and older unencrypted backups. */
async function secureRestoreBackupFromFile(file){
  ...
  if(parsed.type === "encrypted-backup"){
    const password = getBackupPassword();
    if(!password || password.length < 4){
      return alert("Enter the backup password");
    }
    try{
  ...
  }else if(!confirm("This is an older unencrypted backup. Restore it anyway?")){
    return;
  }

  if(!backupData || (backupData.appName !== "My Dear Diary" && backupData.app !== "My Dear Diary")){
    return alert("Invalid My Dear Diary backup file");
  }

  if(!confirm("Restore this backup? Existing memories will be kept and backup memories will be added.")){
    return;
  }

  setBackupStatus("Restoring encrypted backup...");
  let restored = 0;
  const diaryEntries = backupData.diaryEntries || backupData.entries || [];
  const vaultEntries = backupData.vaultEntries || [];
  const timeCapsules = backupData.timeCapsules || [];

  restored += await restoreBackupCollection("diaryEntries", diaryEntries);
  restored += await restoreBackupCollection("vaultEntries", vaultEntries);
  restored += await restoreBackupCollection("timeCapsules", timeCapsules);

  if(Array.isArray(backupData.localTimeCapsules) && backupData.localTimeCapsules.length){
    const existing = getLocalCapsules();
    const imported = backupData.localTimeCapsules.map(item => ({
      ...item,
      id:"local_capsule_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8),
      uid:currentUser.uid,
      localOnly:true
    }));
    saveLocalCapsules(existing.concat(imported));
    restored += imported.length;
  }

  restoreBackupLocalStorage(
    backupData.localStorage,
    backupData.sourceUid || backupData.uid || ""
  );

  if(backupData.design){
    if(backupData.design.diaryTheme) localStorage.setItem("diaryTheme", backupData.design.diaryTheme);
    if(backupData.design.primaryColor) localStorage.setItem("primaryColor", backupData.design.primaryColor);
    if(backupData.design.accentColor) localStorage.setItem("accentColor", backupData.design.accentColor);
    if(backupData.design.bgColor) localStorage.setItem("bgColor", backupData.design.bgColor);
    if(backupData.design.fontChoice) localStorage.setItem("fontChoice", backupData.design.fontChoice);
    if(backupData.design.lampColor) localStorage.setItem("lampColor", backupData.design.lampColor);
  }
  ...
  await updateBackupStats();
  setBackupStatus(`Restore complete. Restored ${restored} item(s).`);
  alert(`Backup restored successfully. ${restored} item(s) added.`);
}
...
if(typeof oldImportDiaryBackupForSecurePassword === "function"){
  window.importDiaryBackup = async function(event){
    const input = event?.target || $("backupImportFile");
    const file = input?.files?.[0];
    if(!file) return alert("Choose a backup file");
    try{
      await secureRestoreBackupFromFile(file);
    }catch(error){
      setBackupStatus("Restore failed.");
      alert("Restore failed: " + error.message);
    }finally{
...
if(typeof oldRestoreDiaryBackupForSecurePassword === "function"){
  window.restoreDiaryBackup = async function(event){
    return window.importDiaryBackup(event);
  };
}
...
function perfRenderMedia(entry){
  ...
  if((entry.mediaType || "").startsWith("video")){
    return `
      <div class="memory-media-card">
        <video controls preload="metadata" src="${escapeHTML(mediaUrl)}" class="memory-media"></video>
      </div>
    `;
  }
  ...
  return `
    <div class="memory-media-card">
      <img loading="lazy" decoding="async" src="${escapeHTML(mediaUrl)}" alt="Memory photo"
        class="memory-media" data-media-url="${escapeHTML(mediaUrl)}"
        onclick="openMediaViewer(this.dataset.mediaUrl, 'image')">
    </div>
  `;
}
...
/* FINAL OLD WORKING VAULT MEDIA METHOD */

function vaultFileToDataUrl(file) {
...
window.saveVaultEntry = async function () {
  ...
  if (mediaFile && mediaFile.size > 1800000) {
    return alert("Photo/video must be smaller than 1.8 MB");
  }

  try {
    const mediaUrl = mediaFile
      ? await vaultFileToDataUrl(mediaFile)
      : "";
    ...
    alert(savedToCloud
      ...
      : "Encrypted secret saved locally"
    );
